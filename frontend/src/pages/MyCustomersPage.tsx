...
export function MyCustomersPage() {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setData({
            data: list,
            meta: res.data?.meta ?? { total: list.length, page: 1, total_pages: 1 },
          })
        }
      } catch (err) {
        if (!cancelled) {
          setError(getApiErrorMessage(err))
          setData({ data: [], meta: { total: 0, page: 1, total_pages: 1 } })
        }
      } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="My Customers"
        description="Customers in your portfolio or assigned territory"
        breadcrumbs={[
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" asChild className="gap-1.5 shadow-xs">
              <Link to="/business-admin/customers/create">
