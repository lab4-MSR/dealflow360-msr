    ...
    <div className="space-y-6">
      ...
      <AnalyticsSection title="Revenue" description="MRR trend, ARR trend, and recurring revenue." isLoading={isLoading} isEmpty={!isLoading && !s.growth} error={error} onRetry={() => refetch()}>
        <ChartFrame ariaLabel="Subscription revenue trend chart" emptyTitle="No revenue data for this period.">
          <ResponsiveContainer width="100%" height="100%"><AreaChart data={[(s.growth ?? {})]} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}><defs><linearGradient id="subRev" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(var(--primary))" stopOpacity={0.3} /><stop offset="95%" stopColor="hsl(var(--primary))" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis stroke="hsl(var(--muted-foreground))" fontSize={12} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} /><Tooltip /><Area type="monotone" dataKey="mrr" stroke="hsl(var(--primary))" fill="url(#subRev)" name="MRR" /></AreaChart></ResponsiveContainer>
        </ChartFrame>
      </AnalyticsSection>
