    ...
    <div className="space-y-6">
      ...
      <AnalyticsSection title="Revenue Trend" description="Daily, monthly, and yearly revenue trends." action={<Tabs value={trendPeriod} onValueChange={setTrendPeriod}><TabsList><TabsTrigger value="daily">Daily</TabsTrigger><TabsTrigger value="monthly">Monthly</TabsTrigger><TabsTrigger value="yearly">Yearly</TabsTrigger></TabsList></Tabs>} isLoading={isLoading} isEmpty={!isLoading && !s.trend_data} error={error} onRetry={() => refetch()}>
        <ChartFrame ariaLabel={`${trendPeriod} revenue trend chart`} emptyTitle="No revenue trend data for this period.">
          <ResponsiveContainer width="100%" height="100%"><AreaChart data={s.trend_data ?? []} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}><defs><linearGradient id="revTrend" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(var(--primary))" stopOpacity={0.3} /><stop offset="95%" stopColor="hsl(var(--primary))" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="period" stroke="hsl(var(--muted-foreground))" fontSize={12} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} /><Tooltip /><Area type="monotone" dataKey="revenue" stroke="hsl(var(--primary))" fill="url(#revTrend)" name="Revenue" animationDuration={350} animationEasing="ease-out" /></AreaChart></ResponsiveContainer>
        </ChartFrame>
      </AnalyticsSection>
